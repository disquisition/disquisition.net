...
async function getUnsplashImage(unsplashPhotoId) {
  const width = 720;
...
module.exports = async (file, content) => {
  ...
  const post = Object.assign({}, results, {
    content: toAST(results.__content),
    html: toHTML(results.__content),
    image: await getUnsplashImage(results.unsplash_photo_id),
    date: toDate(file),
    slug: toSlug(file),
    timestamp: toTimestamp(file),
    title: toTitle(results.__content)
  });
